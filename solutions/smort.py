...
def smort(file: str,
    ...
    for i, combo in enumerate(combos):
        ...
        dist1 = {(pt.x - middle_point.x)**2 + (pt.y - middle_point.y)**2: pt
                 for pt in points1}
        dist2 = {(pt.x - middle_point.x)**2 + (pt.y - middle_point.y)**2: pt
                 for pt in points2}
        ...
        if points1 and points2:
            ...
            if dist_vec_1_cleaned and dist_vec_2_cleaned and\
                    dist_vec_1_cleaned.intersection(dist_vec_2_cleaned):
    ...
    for rhomb in rhomuses:
        try:
            squares.append(Square(rhomb.point1, rhomb.point2,
...
def calc_perp_vectors(side: tuple[Point, Point],
    ...
    while (bound[0][0] <= current_x <= bound[0][1])\
            and (bound[1][0] <= current_y <= bound[1][1]):
        points.append(Point(round(current_x, 2), round(current_y, 2)))
        current_x += perp_unit[0] * 0.5
        current_y += perp_unit[1] * 0.5
